...
blandultim <- function(data, x, y, nboot=NULL, fixed=NULL, random=NULL, bootstrap=NULL, repeated=NULL){
  ...
  stats <- blandultim::parabootstraptLOA(nboot=nboot, data=data, x=x, y=y, seed=859, fixed=fixed, random=random)
  ...
  stats <- blandultim::bland_altman_stats(df=data, x=x, y=y, random=random, fixed=fixed, repeated=repeated)
